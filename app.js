...
const updatePage = async () => {
  ...
  const fruitnames = fruitsArray.filter(filterfunction)
  function filterFunctwo(obj){
    return obj.name
  }
  const nameArr = fruitnames.map(filterFunctwo)
  // TODO: Create a new HTML element to display your data
  const newEl = document.createElement('div');
  newEl.innerHTML = nameArr;
  // TODO: Append your new element to the page
  const exEl = document.getElementById('fruits')
  exEl.append(newEl);
